  ...
  describe "existing short_url instance" do
    ...
    context "with a higher id" do
      ...
      it "has the short_code for id 1001" do
        ...
        expect(short_url.short_code).to eq("f8")
      end

      it "has the short_code for id for 50" do
        short_url.update_column(:id, 50)
        expect(short_url.short_code).to eq("N")
      end

      it "has the short_code for id for 11" do
        short_url.update_column(:id, 11)
        expect(short_url.short_code).to eq("a")
      end

      it "has the short_code for id for 61" do
        short_url.update_column(:id, 61)
        expect(short_url.short_code).to eq("Y")
      end

      it "has the short_code for id for 62" do
        short_url.update_column(:id, 62)
        expect(short_url.short_code).to eq("Z")
      end

      it "has the short_code for id for 63" do
        short_url.update_column(:id, 63)
        expect(short_url.short_code).to eq("00")
      end

      it "has the short_code for id for 3906" do
        short_url.update_column(:id, 3906)
        expect(short_url.short_code).to eq("ZZ")
      end

      it "has the short_code for id for 3907" do
        short_url.update_column(:id, 3907)
        expect(short_url.short_code).to eq("000")
      end

      it "has the short_code for id for 3969" do
        short_url.update_column(:id, 3969)
        expect(short_url.short_code).to eq("010")
      end

      it "has the short_code for id for 7751" do
        short_url.update_column(:id, 7751)
        expect(short_url.short_code).to eq("100")
      end

      it "has the short_code for id for 15018570" do
        short_url.update_column(:id, 15018570)
        expect(short_url.short_code).to eq("ZZZZ")
      end

      it "has the short_code for id for 15018571" do
        short_url.update_column(:id, 15018571)
        expect(short_url.short_code).to eq("00000")
      end
    end
